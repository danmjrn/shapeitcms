...
trait AccessControlTrait
{
    ...
    {
        if($user instanceof  ExternalUser || $user instanceof InternalUser) {
            if ( ! $this->isUser( $user, $userType ) )
                throw $this->createAccessDeniedException();
     ...
     * @param string $userType
     */
    public function validateUserRoleAccess(UserInterface $user, string $role, string $userType = InternalUser::class): void
    {
        if (! $this->isUser($user, $userType))
            throw $this->createAccessDeniedException();
